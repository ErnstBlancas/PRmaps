...
def multi_plot(input_data,elements,output,robust,max):
    ...
    nplots=len(element_list)
    xticks=np.linspace(0,data['Freq'].max(),5)
    ...
    for ax, j in zip(axs.flat, element_list):
        ...
        if robust=='True':
            print('no caca')
            im=ax.imshow(matrix,extent=extent, rasterized=True, interpolation='bicubic',
                         cmap='Blues',origin='lower',aspect='auto',vmin=Scr.min(),vmax=Scr.max())
        if robust=="False":
            print(max)
            im=ax.imshow(matrix,extent=extent, rasterized=True, interpolation='bicubic',
                         cmap='Blues',origin='lower',aspect='auto',vmin=Scr.min(),vmax=1000000)
        ax.set_yticks([])
        ax2=ax.twinx()
    ...
    cax = fig.add_axes([left, 0.07, width, height * 0.05])
...
parser.add_argument('--heatmap_max', type=int, default=None, required=False,
                    help='Heatmap colorbar maximum value')
parser.add_argument("--robust", nargs='?', type=str,
                        default='True',
                        help='If true heatmap range is fixed to  the extreme values')
...
multi_plot(data,elements=element_list,output='multiplot',robust=args.robust,max=args.heatmap_max)
